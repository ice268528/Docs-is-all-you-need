...
| Codex plugin marketplace candidate | Candidate target for Codex Desktop marketplace/plugin installation without polluting Claude Code plugin paths. | The repository-root `marketplace.json` points to `./plugins/diayn`, and `plugins/diayn/` carries the 12 workflow skills plus 23 dependency skills. Codex Desktop should add the repository root with the sparse path field left empty. This fixes the earlier nested-manifest candidate shape, but it is still candidate packaging until Codex Desktop install, discovery, skill invocation, and dependency routing evidence is recorded. |
...
- an isolated Codex plugin candidate under `marketplace.json` and `plugins/diayn/`;
