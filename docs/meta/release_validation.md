...
attempted and not claimed. The external evidence validator remains available as
optional future Desktop runtime evidence tooling.
...
| Codex plugin local candidate | `package_install_validated_app_session_runtime_not_attempted` | `plugins/docs-is-all-you-need/`; `packages/codex-project-local/`; `validation/phase9_codex_project_local_package.json`; `validation/phase9_codex_project_local_install_fixture.json`; `validation/phase9_codex_home_install_fixture.json`; `validation/phase9_codex_runtime_external_evidence.json`; `validation/phase9_codex_runtime_external_evidence_selftest.json`; `docs/install/codex_plugin_local_candidate.md` | Current DDDV8 package artifacts include 12 public workflow skills and 23 DIAYN-managed dependency skills. Static validation, the project-local install fixture, the Codex-home install fixture, and directory inspection pass. The runtime evidence validator remains optional future tooling; app-session discovery/execution is not claimed. |
...
| Codex plugin local candidate | `package_install_validated_app_session_runtime_not_attempted` | Current DDDV8 has a repository-root `.codex-plugin/plugin.json` pointing at the generated platform-visible skills package, plus the inner local candidate, project-local/Home skills package, and committed install-fixture evidence. | Do not claim Desktop app-session discovery/execution without separate runtime evidence. |
