...
`docs/meta/diayn_v1_completion_audit.md`. As of the DDDV8 audit, the Claude
project-local package is the only supported alpha surface because its installed
flow and focused side scenarios are complete. Codex project-local
`.codex/skills` package shape plus project-local and Codex-home install fixtures
are validated. Real Codex Home installs and executable probes are local-only
diagnostics by default, not committed remote release evidence. The external
evidence validator has a selftest proving that complete concrete evidence with
existing repo-relative evidence files clears the blocker while placeholder,
missing, or nonexistent evidence remains blocked. Codex runtime
discovery/invocation still remains blocked until the current or reloaded Codex
app session proves direct
`/diayn-*` invocation and native dependency-skill invocation. That blocks Codex
and all-surface release claims, not the proven Claude project-local surface.
...
| Codex Skills live install/discovery | `file_install_validated_runtime_blocked` | `docs/install/codex_skills.md`; `DDDV6/stage_outputs/d6_03/d6_03_codex_install_log.md`; D6-03 discovery evidence | Historical D6 copied the old role-skill set into a real local Codex skills directory. Current DDDV8 supersedes that with `packages/codex-project-local/` and validated the package plus install-fixture shapes. Runtime discovery or execution from the current/reloaded Codex app session is still not verified. |
| Codex plugin local candidate | `static_package_and_install_validated_runtime_blocked` | `plugins/docs-is-all-you-need/`; `packages/codex-project-local/`; `validation/phase9_codex_project_local_package.json`; `validation/phase9_codex_project_local_install_fixture.json`; `validation/phase9_codex_home_install_fixture.json`; `validation/phase9_codex_runtime_external_evidence.json`; `validation/phase9_codex_runtime_external_evidence_selftest.json`; `docs/install/codex_plugin_local_candidate.md` | Current DDDV8 package artifacts include 12 public workflow skills and 23 DIAYN-managed dependency skills. Static validation, the project-local install fixture, the Codex-home install fixture, the external evidence intake validator, and the validator selftest pass, including rejection of nonexistent evidence references. Real Codex Home installs are local-only diagnostics unless sanitized and promoted. App-session discovery/execution is not verified and must not be claimed as `working`. |
...
| --- | --- | --- | --- |
| Codex Skills live discovery | `file_install_validated_runtime_blocked` | Historical D6 copied the old role-skill set into a maintainer's local Codex skills directory. Current DDDV8 has rebuilt the package around the 12 public workflow skills and 23 dependency skills, with committed project-local and Codex-home install fixtures. | Record current/reloaded Codex app-session discovery and execution evidence before upgrading to `working`. |
| Codex plugin local candidate | `file_install_validated_runtime_blocked` | Historical D6 created `plugins/docs-is-all-you-need/` with a local `.codex-plugin/plugin.json` and the old role-skill set. Current DDDV8 has rebuilt the candidate around 12 workflow skills plus a project-local/Home skills package and committed install-fixture evidence. | Keep runtime support blocked until the current or reloaded Codex app session observes direct `/diayn-*` discovery/execution and native dependency-skill invocation. |
