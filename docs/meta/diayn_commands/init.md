...
Resolve the adapter platform before creating scaffold files:

| Platform | Entry file |
| --- | --- |
| Claude Code plugin or project-local fallback | `CLAUDE.md` |
| Codex | `AGENTS.md` |
| OpenCode | `AGENTS.md` |
| Generic AGENTS.md-based adapter | `AGENTS.md` |

Do not default-generate both entry files. If the other entry file already exists, preserve it and record it in the harness audit as an existing cross-agent file.
...
The audit should report platform, selected entry file, why the other entry file was not generated by default, Git/non-Git status, dirty working tree, missing baseline files, existing-file conflicts, generated or large-file scan boundaries, nested repositories, possible secret-bearing file names without secret values, document language inference, and OwnerGate items.
...
- selected platform entry file: `CLAUDE.md` for Claude Code; `AGENTS.md` for Codex/OpenCode/generic
- `TODO.md`
...
- `.diayn/dependency-routing/upstream-routing-map.md` when initializing DIAYN-managed dependency-skill routing.
