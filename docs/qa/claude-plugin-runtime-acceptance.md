...
/diayn:diayn-* or /diayn-* workflow skill entries should not be visible as user workflow entries in plugin mode.
...
If Claude exposes `/diayn-*` bare DIAYN workflow entries in plugin mode without
a separate project-local fallback installed, record it as a runtime acceptance
failure.
