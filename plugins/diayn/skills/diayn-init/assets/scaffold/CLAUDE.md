...
This is the Claude Code landing page for this project.

This project uses DIAYN as a document-driven workflow for multi-session coding-agent work. In Claude Code plugin mode, prefer `/diayn:*`; bare `/diayn-*` belongs to Claude project-local fallback. This file is the Claude Code peer entry file. `AGENTS.md` is the Codex/OpenCode/generic peer entry file. Neither one is a wrapper or mandatory first read for the other.
