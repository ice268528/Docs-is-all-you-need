...
function readPluginCommandDescription(skillName) {
  const commandPath = path.join(pluginRoot, ".claude", "commands", `${skillName}.md`);
  const text = fs.readFileSync(commandPath, "utf8").replace(/^\uFEFF/, "");
  const match = text.match(/^---\r?\ndescription: (.*?)\r?\n---/);
  if (!match) throw new Error(`missing command description for ${skillName}`);
  return match[1];
}
...
function commandAdapter(skillName) {
  const description = readPluginCommandDescription(skillName);
