...
4. Read only the platform entry docs needed to answer the cold-start questions. Do not load both `AGENTS.md` and `CLAUDE.md` by default; inspect the other entry file only when it already exists and matters to a conflict or cross-agent boundary.
...
9. Do not default-generate both entry files. If the other entry file already exists, preserve it and record it as an existing cross-agent entry file in the harness audit instead of updating it.
